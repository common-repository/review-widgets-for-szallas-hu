<?php
class TrustindexPlugin_szallashu
{
...
$chosedPlatform = new $className($forcePlatform, $filePath, "do-not-care-12.4.1", "do-not-care-Review Widgets for Szallas.hu", "do-not-care-Szállás.hu");
...
$split = '/';
$replaceHu = false;
if ($this->getShortName() === 'arukereso') {
$split = '|';
$replaceHu = true;
}
$tmp = explode($split, $pageDetails['id']);
$content = str_replace('icon.svg', 'icon-'.$tmp[0].'.png', $content);
$content = str_replace(['img/platform/logo.svg', 'img/platform/logo-dark.svg'], ['img/platform/logo-'. $tmp[0] .'.svg', 'img/platform/logo-'. $tmp[0] .'-dark.svg'], $content);
$content = str_replace(['platform/'. ucfirst($this->getShortName()) .'/logo.svg', 'platform/'. ucfirst($this->getShortName()) .'/logo-dark.svg'], ['platform/'. ucfirst($this->getShortName()) .'/logo-'. $tmp[0] .'.svg', 'platform/'. ucfirst($this->getShortName()) .'/logo-'. $tmp[0] .'-dark.svg'], $content);
if ($replaceHu) {
$content = str_replace('img/platform/logo-hu', 'img/platform/logo', $content);
$content = str_replace('platform/'. ucfirst($this->getShortName()) .'/logo-hu', 'platform/'. ucfirst($this->getShortName()) .'/logo', $content);
}
